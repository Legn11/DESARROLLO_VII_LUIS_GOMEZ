...
function crecimientoVentas($ventas) {
    $crecimientos = [];
    $mayorCrecimiento = ['region' => '', 'producto' => '', 'crecimiento' => -INF];
    ...
    foreach ($ventas as $region => $productos) {
        foreach ($productos as $producto => $ventasProducto) {
            ...
            if ($porcentaje > $mayorCrecimiento['crecimiento']) {
                $mayorCrecimiento = [
                    'region' => $region,
                    'producto' => $producto,
    ...
    return [$crecimientos, $mayorCrecimiento];
}

[$crecimientosPorRegion, $maxCrecimiento] = crecimientoVentas($ventas);

echo "\nCrecimiento de ventas (%) del primer al último mes por región y producto:\n";
foreach ($crecimientosPorRegion as $region => $productos) {
    echo "$region:\n";
...
echo "\nProducto y región con mayor crecimiento: {$maxCrecimiento['producto']} en {$maxCrecimiento['region']} ({$maxCrecimiento['crecimiento']}%)\n";
